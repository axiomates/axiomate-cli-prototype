...
const KEYWORD_TO_TOOL: Record<string, string[]> = {
	...
	python: ["python", "pip", "pyenv", "conda", "poetry", "requirements.txt"],
	java: ["java", "javac", "maven", "gradle", "mvn"],

	// 构建工具
	cmake: ["cmake", "CMakeLists", "make"],
	gradle: ["gradle", "gradlew"],
	maven: ["maven", "mvn", "pom.xml"],

	// 容器
	docker: ["docker", "dockerfile", "container", "compose", "容器"],
	docker_compose: ["docker-compose", "docker compose", "compose.yml"],
	...
	vs2022: ["visual studio", "msbuild", "sln", "csproj"],

	// 比较工具
	beyondcompare: ["beyond compare", "diff", "compare", "merge files"],
};
...
function getPlatformShellTool(): string {
	if (platform() === "win32") {
		return "powershell"; // Windows 默认使用 PowerShell
	}
	return "bash"; // Unix/Linux/macOS 使用 bash
}
...
export function buildToolMask(
	...
	// 备选 shell
	if (platform() === "win32") {
		if (availableToolIds.has("cmd")) {
			allowedTools.add("cmd");
		}
		if (availableToolIds.has("pwsh")) {
			allowedTools.add("pwsh");
		}
	}
